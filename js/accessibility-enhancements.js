...
function addAccessibilityToolbar() {
    ...
    toolbar.innerHTML = `
        <button 
            class="a11y-btn" 
            onclick="HighContrastMode.toggle()"
            aria-label="${isRTL ? 'تبديل التباين العالي' : 'Toggle high contrast'}"
            title="${isRTL ? 'تباين عالي' : 'High Contrast'}"
        >
            <i class="fas fa-adjust"></i>
        </button>
        <button 
            class="a11y-btn" 
            onclick="document.body.style.fontSize = parseInt(getComputedStyle(document.body).fontSize) + 2 + 'px'"
            aria-label="${isRTL ? 'تكبير النص' : 'Increase text size'}"
            title="${isRTL ? 'تكبير النص' : 'Increase Text'}"
        >
        ...
        <button 
            class="a11y-btn" 
            onclick="document.body.style.fontSize = parseInt(getComputedStyle(document.body).fontSize) - 2 + 'px'"
            aria-label="${isRTL ? 'تصغير النص' : 'Decrease text size'}"
            title="${isRTL ? 'تصغير النص' : 'Decrease Text'}"
        >
